...
public class AggregateService {
    ...
    public Aggregate<?, ?> createBaselineAggregate(AggregateKey key) {
        if (key instanceof AddressAggregate.AddressAggregateKey) {
            AddressAggregate.AddressAggregateKey addressAggregateKey = (AddressAggregate.AddressAggregateKey) key;
            String tokenContractHash = addressAggregateKey.getTokenContractHash();
            String address = addressAggregateKey.getAddress();
            if (Constants.ONG_CONTRACT_HASH.equals(tokenContractHash) && address.startsWith(Constants.EVM_PREFIX)) {
                String ethAddress = HttpUtil.ethAddrToOntAddr(address);
                addressAggregateKey.setAddress(ethAddress);
            }
            return createBaselineAggregate(addressAggregateKey);
